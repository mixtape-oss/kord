            ...
            with(decoder.beginStructure(descriptor)) {
                loop@ while (true) {
                    when (val index = decodeElementIndex(descriptor)) {
                        ...
                        1 -> data = when (op) {
                            OpCode.Hello ->
                                decodeSerializableElement(descriptor, index, Hello.serializer())

                            OpCode.HeartbeatAck ->
                                HeartbeatAck(decodeInlineElement(HeartbeatAck.serializer().descriptor, 0).decodeLong())

                            OpCode.Ready ->
                                decodeSerializableElement(descriptor, index, Ready.serializer())

                            OpCode.SessionDescription ->
                                decodeSerializableElement(descriptor, index, SessionDescription.serializer())

                            OpCode.Speaking ->
                                decodeSerializableElement(descriptor, index, Speaking.serializer())
